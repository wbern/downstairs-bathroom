...
- **The room has no windows**, so there is no sun to model. It is lit entirely by its own fittings (`addRoomLight`): five ceiling spots with visible recessed trims (only the middle one casts shadows — the rest are far too expensive), a mirror ring, LED under the vanity and in the wall cabinet, and a shower-niche glow. `applyLighting()` owns `scene.environmentIntensity`, so `applyPalette()` calls it rather than setting the value itself.
