...
- **The room is lit by its own fittings** (`addRoomLight`): five ceiling spots (only the middle one casts shadows — the rest are far too expensive), a mirror ring, LED under the vanity and in the wall cabinet, and a shower-niche glow. Two moods: **Dag** (sun + fittings) and **Kväll** (fittings only). `setMood()` owns `scene.environmentIntensity`, so `applyPalette()` calls it rather than setting the value itself.
...
- **`CEMENTS` + `setCement()`** switch the wall/floor microcement colour live: Macchiato plus the three shortlisted Jotun beiges and the three contrast colours from the dossier's Kulörjämförelse. Screen approximations of the NCS notations, not colour proofs. The dossier's colour section links straight here.
